...
function renderPanel(tab=activeTab){
 ...
 const a=state.asset;
 if(tab==='overview'){
  $('#panel-content').innerHTML=`<h3>Informasi mesin</h3><dl class="data-list">${pair('Kode aset',a.asset_code)+pair('Kode nama',a.codename)+pair('Pabrikan',a.manufacturer)+pair('Model',a.model)+pair('Kategori','Mesin produksi')+pair('Spesifikasi',a.specification)+pair('Lokasi',a.location)+pair('Status operasi',null)}</dl><h3>Model 3D</h3><div class="card accent"><h4>Model berdasarkan foto mesin</h4><p>Feeder, cover, area roller, bak tinta, pijakan, platform, dan delivery dibuat dengan mengacu pada foto aktual dan dokumen mesin yang tersedia.</p><span class="tag">FOTO AKTUAL</span><span class="tag">DOKUMEN MESIN</span></div><dl class="data-list">${pair('Dimensi aktual',null)+pair('Susunan unit','8 unit ditampilkan')+pair('Posisi di pabrik',activeLayout()?'Tersedia pada denah':'Belum tersedia')+pair('Dokumen identitas',a.sources.length+' dokumen')}</dl><div class="card"><h4>Catatan penggunaan</h4><p>Beberapa ukuran dan detail internal belum memiliki data pasti. Gunakan model untuk eksplorasi visual dan pemahaman struktur mesin.</p></div>`;
