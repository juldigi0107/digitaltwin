...
test('every printing unit carries the detailed service, pneumatic and inspection geometry',()=>{
...
test('feeder through delivery deep-detail nodes resolve through six-stage taxonomy',()=>{
  const model=new OffsetMachineTemplate();
  for(const id of ['feedboard-lay-mechanism','feeder-suction-cups','feeder-separator-brushes','coater-blade-adjusters','pu8-coater-access','coater-dryer-service-bay','dryer-monitoring','inspection-trigger','dryer-delivery-access','delivery-chain-tensioners','delivery-pile-lift'])assert.ok(model.findNode(id),`missing ${id}`);
  for(let n=1;n<=7;n++)assert.ok(model.findNode(`transfer-pu${n}-pu${n+1}-gripper-spring`),`transfer ${n}/${n+1} missing spring`);
  for(const id of ['O5.FEEDER.SEP.B1.P3','O5.PRINT.PU1.COVER.PNEU.S1','O5.PRINT.PU8.COVER.INSPECT.S1','O5.COATER.ACCESS.B1.P3.S1','O5.DRYER.SERVICE.B1.P3.S1','O5.INSPECTION.CONTROL.B1.P4.S1','O5.DELIVERY.ACCESS.B1.P3.S1']){
