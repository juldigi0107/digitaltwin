...
test('source registry separates photo evidence from technical reference',()=>{
 const stats=photoStats();assert.equal(PHOTO_REGISTRY.length,16);assert.equal(stats.unique,16);assert.equal(stats.active_geometry_reference,9);
 assert.ok(TECHNICAL_SOURCES.some(s=>s.publisher.includes('Heidelberger')));assert.equal(ORIENTATION.feedDirection,'FEEDER_TO_DELIVERY_POSITIVE_X');assert.equal(ORIENTATION.driveSide,'UNVERIFIED');
});
